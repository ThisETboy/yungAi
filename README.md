...
│   │   ├── annotation/            # @OperateLog, @RateLimit
│   │   ├── aspect/                # AOP 切面
...
│   ├── module/
│   │   ├── system/                # 系统模块（RBAC）
│   │   │   ├── controller/        # Auth / User / Role / Menu / Profile / Cache / Config / Dict / File / OperLog / LoginLog / OnlineUser
│   │   │   ├── service/           # 业务逻辑（含 CaptchaService、TokenBlacklist、OnlineUser、UserInfoCache 等）
│   │   │   ├── entity/            # 数据库实体
│   │   │   ├── dto/               # 请求数据传输对象
│   │   │   ├── vo/                # 视图对象
...
│   │   │   │   └── AgnesAiAdapter.java
│   │   │   ├── controller/        # AiChatController / AiModelConfigController
...
│   │   │   ├── controller/RequestLogController.java
...
└── frontend/                      # Vue 3 前端
    ...
    │   │   ├── login/             # 登录页（验证码 + 记住我）
    │   │   ├── dashboard/         # 仪表盘
    │   │   ├── system/            # 系统管理（用户/角色/菜单/字典/文件/配置/缓存/日志/在线用户）
    │   │   ├── ai/                # AI 功能（聊天/代码生成/模型配置）
    │   │   ├── cloud/             # 词云中心
    │   │   ├── protocol/          # 协议管理
    │   │   ├── monitor/           # 请求日志
    │   │   ├── profile/           # 个人中心
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `sys.captcha.length` | 随机码长度 | 4 |
| `sys.captcha.width` | 图片宽度 | 120px |
| `sys.captcha.height` | 图片高度 | 40px |
| `sys.captcha.ttl` | TTL（分钟） | 5 |

> 注：前端显示尺寸固定为 120x40px，与后端配置保持一致。
...
**Q: 前端开发服务器端口 5174 被占用？**
```
A: 修改 vite.config.ts 中的 server.port 配置（默认 5174）
...
> `sys_online_user` 不在数据库中，在线用户记录由 Redis 管理（`online:user:{userId}`）。
...
API 按模块分为 9 组：认证管理、用户管理、角色管理、菜单管理、AI 功能、协议管理、词云中心、系统管理（字典/文件/配置/缓存/操作日志/登录日志/在线用户）、监控日志。
...
- **在线用户**: 记录登录用户 Token，支持强制下线
- **登录锁定**: 连续登录失败 5 次锁定账号 30 分钟
...
**模型配置**: 支持动态添加/编辑/删除 AI 提供商模型，前端可切换当前使用的模型。
...
| `sys:ai:model` | AI 模型管理 | GET/POST/PUT/DELETE /api/ai/model-configs, 模型切换 |
...
| `sys:loginlog:list` | 登录日志查看 | GET /api/auth/login-logs |
| `sys:cloud:list` | 词云查看 | GET /api/cloud/words, /api/cloud/categories, /api/cloud/words/page |
...
| `sys:online:user:list` | 在线用户查看 | GET /api/online-users |
| `sys:online:user:off` | 强制下线 | DELETE /api/online-users/{userId}/force-offline |
...
| 页面 | 路由 | 对应后端接口 | 功能 |
|------|------|-------------|------|
| 登录 | `/login` | `GET /api/auth/captcha` + `POST /api/auth/login` | 图形验证码 + 用户名密码登录，支持"记住我" |
| 首页 | `/dashboard` | `GET /api/users`, `/roles`, `/ai/models`, `/protocol/status` | 统计概览 |
| 用户管理 | `/system/user` | `GET/POST/PUT/DELETE /api/users` | 用户 CRUD + 分页 + 搜索 + 分配角色 |
| 角色管理 | `/system/role` | `GET/POST/DELETE /api/roles` | 角色 CRUD + 菜单权限分配 |
| 菜单管理 | `/system/menu` | `GET /api/menus/tree, POST/PUT/DELETE /api/menus` | 菜单树 CRUD + 名称搜索 |
| AI 聊天 | `/ai/chat` | `POST /api/ai/chat/stream`, `GET /api/ai/model-configs` | SSE 流式聊天 + Markdown 渲染 |
| 代码生成 | `/ai/codegen` | `POST /api/ai/chat/stream` | AI 驱动代码生成 |
| AI 模型配置 | `/ai-models` | `GET/POST/PUT/DELETE /api/ai/model-configs` | AI 模型 CRUD 管理 |
...
| 个人中心 | `/profile` | `GET/PUT /api/profile/*` | 修改密码 + 修改资料 + 修改头像 |
| 文件管理 | `/files` | `GET /api/files/list, POST /api/files/upload` | 文件上传 + 下载预览 |
| 系统配置 | `/config` | `GET/POST/DELETE /api/config` | 运行时配置管理 |
| 缓存管理 | `/cache` | `GET/DELETE /api/cache/*` | Redis 缓存可视化管理 |
| 登录日志 | `/login-log` | `GET /api/auth/login-logs` | 登录日志分页查询 |
| 在线用户 | （动态菜单） | `GET /api/online-users` | 查看在线用户 + 强制下线 |
...
| 来源 | 端口 | 用途 |
|------|------|------|
| `http://localhost:5174` | 5174 | Vite 开发服务器（vite.config.ts 已配置） |
...
**TODO/FIXME 修复**
- 修复 OperateLogAspect 硬编码 IPv6 回环地址，改为从请求头提取真实客户端 IP
- 修复词云时间范围筛选未实现问题，前后端均已支持 `startDate`/`endDate` 参数
- 修复 AI 关键词分类写死为"科技"的问题，改为使用首个可用分类
- 修复文件上传后列表不刷新问题，新增 `loadFileList()` 函数

**后端新增**
- 文件列表查询接口 `GET /api/files/list`（分页 + 关键词搜索）
- 文件管理服务 `FileService.listFiles()` 及实现

**前端新增**
- 文件 API 模块 `frontend/src/api/file.ts`
- 文件管理页面上传成功后自动刷新列表

### v0.5.0 — 2026-07-21

**新增功能**
- 登录验证码（图形随机码）— 后端 AWT 生成 PNG，SHA-256 哈希存入 Redis，一次性校验
- 新增 `GET /api/auth/captcha` 接口返回验证码图片 Base64
- 登录请求增加 `captchaUuid` + `captchaCode` 字段

**安全增强**
- 登录前强制校验验证码，失败直接拒绝并记录日志
- 验证码 5 分钟过期，校验成功后立即删除（防重放）
- 字符池排除易混淆字符（O/0/I/l/1），支持大小写不敏感输入

**前端变更**
- 登录页增加验证码输入框和图片刷新按钮
- 新增 `CaptchaResponse` 类型定义和 `getCaptcha()` API
- 登录失败自动刷新验证码
...
- `GET/POST/PUT/DELETE /api/ai/model-configs` — AI 模型配置 CRUD
